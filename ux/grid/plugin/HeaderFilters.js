 ...
 * @version 0.2.0
 */
Ext.define('App.ux.grid.plugin.HeaderFilters',{
    
    ptype: 'gridheaderfilters',
    
    alternateClassName: ['App.ux.grid.HeaderFilters', 'App.ux.grid.header.Filters'],
    
    requires: [
        'Ext.container.Container',
        'Ext.tip.ToolTip',
        'App.ux.grid.header_filters.FilterField'
    ],
   ...
   */
   ...
   init: function(grid)
   {
            ...
            if(column.filter)
            {
                ...
                for(var ci = 0; ci < fca.length; ci++)
                {
                    ...
                    Ext.apply(fc, {
                        ...
                        listeners : {
                            scope : this,
                            change : function(fl, val) {
                                var me = this;
                                    
                                fl.tt = true;
                                    
                                setTimeout(function(){
                                    if (Ext.isEmpty(fl.tt) == false) {
    ...
    resetFilters: function(suspendEvent)
    {
        if(!this.fields)
            return;
        for(var fn in this.fields)
        {
            ...
            if (Ext.isFunction(f.clearValueParams)) {
                f.clearValueParams();
                
                if (Ext.isFunction(f.updateTriggers)) {
                    f.updateTriggers();
                }
            }
    ...
    parseFilters: function()
    {
        ...
        for(var fn in this.fields)
        {
            ...
            if (Ext.isEmpty(field.valueParams) == false) {
                filters[field.filterName] = field.getValueParams();    
            } else if (field.isXType('combo') && Ext.isEmpty(field.getValue()[0]) == false) {
                filters[field.filterName] = {"$in" : field.getSubmitValue()};
            } else if(field.isXType('uxFilterField') && Ext.isEmpty(field.getValue()) == false) {
                filters[field.filterName] = {"$search" : field.getSubmitValue()};
            } else {
    ...
    applyFilters: function()
    {
        ...
        if(this.grid.fireEvent('beforeheaderfiltersapply', this.grid, filters, this.grid.getStore()) !== false)
        {
            ...
            for(var fn in filters)
            {
                ...
                //Combobox multiselect fix default value 
                if (Ext.isArray(value) && value[0] == "") {
                    value = "";
                }
